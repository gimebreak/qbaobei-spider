...
class Mysql:
    ...
    def get_acticle_url(self,url):
        result=self.get_data('select url from jiajia_spider_article where url = %s',(url))
        return result
    ...
    def insert_article(self, article_type, title, descriptin, material, content, source, url):
        ...
        if (result == 0):
            insert_sql = "insert into jiajia_spider_article(type, title, description, material, content, source, url) values(%s, %s, %s, %s, %s, %s, %s)"
            return self.insert_data(insert_sql, (article_type, title, descriptin, material, content, source, url))
        return 0

    #获取指定article数据
    def get_article(self, url):
        select_sql = "select * from jiajia_spider_article where url=%s"
        return self.get_data(select_sql, (url))

    #通过“标题”获取指定article数据
    def get_article_by_title(self, title):
        select_sql = "select * from jiajia_spider_article where title=%s"
    ...
    def get_data(self, select_sql, values):
        ...
        try:
            conn,cursor=self.connect()
            result = cursor.execute(select_sql, values)
...
if __name__ == '__main__':
    ...
    url=u'http://www.qbaobei.com/jiankang/2009-2/105304137.html'


    print mysql.get_tag(u'补气血')[0]
    print mysql.get_acticle_url(url)
